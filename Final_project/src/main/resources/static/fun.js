...
function() {
		...
		t.forEach(function(i) {
			...
			for (v = 0; v < w.length; v++) {
				...
				if (i !== x && null !== x.x && null !== x.y) {
					...
					y < x.max && (x === f && y >= x.max / 2 && (i.x -= 0.03 * B, i.y -= 0.03 * z), A = (x.max - y) / x.max, e.beginPath(), e.lineWidth = A / 2, e.strokeStyle = "rgba(" + s.color + ",0.6)", e.moveTo(i.x, i.y), e.lineTo(x.x, x.y), e.stroke())
				}
			}
	...
	u.id = c;
	u.style.cssText = "position:fixed;top:0;left:0;z-index:" + s.z;
